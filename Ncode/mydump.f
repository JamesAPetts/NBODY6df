...
*
      ...
      INCLUDE 'params.h'
      PARAMETER  (NA=84,NB=168,NC=530,ND=392+MLR+MLD+MLV,NE=24,
...
! MGi make a copy of fort.2 before it is overwritten
         if (j.eq.2) then
           call system('if [ -e fort.2 ]; then cp fort.2 fort.2a; fi')
          call system('if [ -e fort.2a ]; then cp fort.2a fort.2b; fi')
         endif


        WRITE (J) ntot,npairs,nttot,a,b,c,d,e,g,l,o,p,q,s
...
*         Jpetts - write galaxy variables to file
        write (J) GMG,(RG(k),k=1,3),(VG(k),k=1,3),(FG(k),k=1,3),
     *    (FGD(k),k=1,3),TG,OMEGA,DISK,AA,BB,V02,RL2,GMB,AR,GAM,
     *    (ZDUM(i),i=1,7)
...
*       Restore standard array pointer.
        ...
        READ (J) ntot,npairs,nttot,a,b,c,d,e,g,l,o,p,q,s
...
*         Jpetts - read in galaxy variables from file
        read (J) GMG,(RG(k),k=1,3),(VG(k),k=1,3),(FG(k),k=1,3),
     *    (FGD(k),k=1,3),TG,OMEGA,DISK,AA,BB,V02,RL2,GMB,AR,GAM,
     *    (ZDUM(i),i=1,7)
